...
def featureSelection(X, y, options=None):
    ...
    if "PLS coefficients" in methods:
        ...
        feature_selected["PLS coefficients"] = np.argsort(np.abs(pls_model.model['B'][1:, n_comp_pls-1]))[::-1][:n_sel]
        variable_importance["PLS coefficients"] = np.abs(pls_model.model['B'][1:, n_comp_pls-1])
    ...
    if "MC-UVE" in methods:
        mc_model = MCUVE(X.to_numpy(), y.to_numpy(), n_comp_pls, nrep=n_rep).fit()
    ...
    FS = pd.DataFrame(data=np.full((len(feature_selected), X.shape[1]), False), index=feature_selected.keys(), columns=wv)
    ...
    Imp = pd.DataFrame(variable_importance, index=wv, columns=feature_selected.keys()).T
...
if "X" in locals() and "y" in locals():
    ...
    with st.container():
        ...
        n_comp_pls = st.slider("Number of components", 1, int(np.min(list(X.to_numpy().shape) + [10])), 1)
        n_sel = st.slider("Number of selected features", 1, X.shape[1], round(X.shape[1] * 0.2))
        n_rep = st.slider("Number of repetitions", 1 * X.shape[1], 10 * X.shape[1], 2 * X.shape[1])
    ...
    options = {"n_comp_pls": n_comp_pls, "n_sel": n_sel, "n_rep": n_rep, "methods": methods}

    FS, Imp = featureSelection(X, y, options)

# Apply the feature selection results to other spectral data uploaded
if "FS" in locals():
    st.markdown("### Apply Feature Selection results to uploaded Spectra")

    fs_method = st.radio("Apply feature selection", FS.index)

    upload_X_new = st.file_uploader("Choose file for NIR spectra to apply feature selection", type="csv")
